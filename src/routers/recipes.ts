...
// add recipes
routerRecipe.post("/add", authenticateToken, async (req: Request, res: Response) => {
  try {
    const authHeader = req.headers["authorization"];
    const token = authHeader && authHeader.split(" ")[1];
    ...
    if (!name || !ingredients) {
      return res.status(400).json({ 'message': 'Nome e ingredienti sono richiesti' });
    }

    
    const verify = await client.query('SELECT * FROM recipes WHERE name = $1 AND ingredients = $2', [name, ingredients]);
    ...
    if (verify.rowCount && verify.rowCount > 0) {
      ...
      if ( result.rowCount && result.rowCount > 0) {
        return res.status(200).json({ 'message': 'Ricetta aggiunta con successo', 'recipe': result.rows[0] });
      } else {
...
// update recipe
routerRecipe.put('/:idrecipe', authenticateToken, async (req: Request, res: Response) => {
  try {
    const authHeader = req.headers["authorization"];
    const token = authHeader && authHeader.split(" ")[1];
    ...
    const verify = await client.query('SELECT * FROM recipes WHERE idrecipe = $1', [idrecipe]);
